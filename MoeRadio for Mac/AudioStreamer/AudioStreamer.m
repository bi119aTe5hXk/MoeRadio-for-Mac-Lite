...
- (BOOL)openReadStream
{
	@synchronized(self)
	{
		...
		if( [[url absoluteString] rangeOfString:@"https"].location != NSNotFound )
		{
			...
			CFReadStreamSetProperty(stream, kCFStreamPropertySSLSettings, ( CFTypeRef)CFBridgingRetain(sslSettings));
		}
...
- (BOOL)isMeteringEnabled {
	...
	if(!status) {
		return (enabled == 1);
        //NSLog(@"MeteringEnabled.");
	}else{
        //NSLog(@"MeteringNOTEnabled.");
        return YES;
    }
}
...
- (void)setMeteringEnabled:(BOOL)enable {
    ...
    NSLog(@"setMeteringEnabled:%d",(int)status);
	if(status)
		return;
...
//	}
    peakPower = audioLevels[channelNumber].mAveragePower;
	free(audioLevels);
    //NSLog(@"avg:%f",audioLevels[channelNumber].mAveragePower);
	return peakPower;
